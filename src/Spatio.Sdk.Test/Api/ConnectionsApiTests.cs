/*
 * SpatioAPI
 *
 * The REST API that owns every resource in your Spatio workspace: notes, sheets, slides, tasks, calendar events, mail, chat, files, and contacts. SpatioMCP wraps this API; Spatio Desktop reads from it. You can call it directly from your own code.  All requests must be authenticated with a Personal Access Token (`Authorization: Bearer pat_...`) or an OAuth 2.1 access token, and use HTTPS.  Official SDKs (MIT, generated from this spec on every release):  - TypeScript: https://github.com/spatio-labs/spatio-ts (`npm install @spatio/sdk-ts`) - Python: https://github.com/spatio-labs/spatio-py (`pip install spatio-sdk`) - Go: https://github.com/spatio-labs/spatio-go (`go get github.com/spatio-labs/spatio-go`)  This specification is generated from the platform-service Go source on every push to `main`. The spec, not hand-written documentation, is the source of truth: server stubs and SDKs are generated from it, and any drift between the spec and the running service fails CI. 
 *
 * The version of the OpenAPI document: v1
 * Contact: hello@spatio.app
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */

using System;
using System.IO;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.Reflection;
using Xunit;

using Spatio.Sdk.Client;
using Spatio.Sdk.Api;
// uncomment below to import models
//using Spatio.Sdk.Model;

namespace Spatio.Sdk.Test.Api
{
    /// <summary>
    ///  Class for testing ConnectionsApi
    /// </summary>
    /// <remarks>
    /// This file is automatically generated by OpenAPI Generator (https://openapi-generator.tech).
    /// Please update the test case below to test the API endpoint.
    /// </remarks>
    public class ConnectionsApiTests : IDisposable
    {
        private ConnectionsApi instance;

        public ConnectionsApiTests()
        {
            instance = new ConnectionsApi();
        }

        public void Dispose()
        {
            // Cleanup when everything is done.
        }

        /// <summary>
        /// Test an instance of ConnectionsApi
        /// </summary>
        [Fact]
        public void InstanceTest()
        {
            // TODO uncomment below to test 'IsType' ConnectionsApi
            //Assert.IsType<ConnectionsApi>(instance);
        }

        /// <summary>
        /// Test DisconnectConnection
        /// </summary>
        [Fact]
        public void DisconnectConnectionTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //DisconnectConnectionRequest disconnectConnectionRequest = null;
            //var response = instance.DisconnectConnection(disconnectConnectionRequest);
            //Assert.IsType<Dictionary<string, Object>>(response);
        }

        /// <summary>
        /// Test InstallConnection
        /// </summary>
        [Fact]
        public void InstallConnectionTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //InstallConnectionRequest installConnectionRequest = null;
            //var response = instance.InstallConnection(installConnectionRequest);
            //Assert.IsType<Dictionary<string, Object>>(response);
        }

        /// <summary>
        /// Test ListAccounts
        /// </summary>
        [Fact]
        public void ListAccountsTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //var response = instance.ListAccounts();
            //Assert.IsType<AccountListResponse>(response);
        }

        /// <summary>
        /// Test ListConnectionIntegrations
        /// </summary>
        [Fact]
        public void ListConnectionIntegrationsTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //var response = instance.ListConnectionIntegrations();
            //Assert.IsType<ConnectionListResponse>(response);
        }

        /// <summary>
        /// Test ListConnections
        /// </summary>
        [Fact]
        public void ListConnectionsTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //var response = instance.ListConnections();
            //Assert.IsType<ConnectionListResponse>(response);
        }

        /// <summary>
        /// Test ListUserConnections
        /// </summary>
        [Fact]
        public void ListUserConnectionsTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //var response = instance.ListUserConnections();
            //Assert.IsType<ConnectionAccountListResponse>(response);
        }

        /// <summary>
        /// Test RefreshConnection
        /// </summary>
        [Fact]
        public void RefreshConnectionTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //RefreshConnectionRequest refreshConnectionRequest = null;
            //var response = instance.RefreshConnection(refreshConnectionRequest);
            //Assert.IsType<Dictionary<string, Object>>(response);
        }

        /// <summary>
        /// Test RemoveAccount
        /// </summary>
        [Fact]
        public void RemoveAccountTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //string accountId = null;
            //instance.RemoveAccount(accountId);
        }

        /// <summary>
        /// Test ResolveAccount
        /// </summary>
        [Fact]
        public void ResolveAccountTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //string? provider = null;
            //string? email = null;
            //var response = instance.ResolveAccount(provider, email);
            //Assert.IsType<Dictionary<string, Object>>(response);
        }

        /// <summary>
        /// Test SyncAccount
        /// </summary>
        [Fact]
        public void SyncAccountTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //string accountId = null;
            //var response = instance.SyncAccount(accountId);
            //Assert.IsType<Dictionary<string, Object>>(response);
        }

        /// <summary>
        /// Test UpdateAccount
        /// </summary>
        [Fact]
        public void UpdateAccountTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //string accountId = null;
            //UpdateAccountRequest updateAccountRequest = null;
            //var response = instance.UpdateAccount(accountId, updateAccountRequest);
            //Assert.IsType<Dictionary<string, Object>>(response);
        }
    }
}
